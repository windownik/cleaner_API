...
# получаем данные с одним фильтром
async def read_data_order(db: Depends, table: str, id_name: str, id_data, name: str = '*'):
    data = await db.fetch(f"SELECT {name} FROM {table} WHERE {id_name} = $1 ORDER BY order_id DESC;", id_data)
    return data
